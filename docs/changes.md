...
2026/9/18
  ...
  * 完善 glyph 中的属性类型(感谢泽武提供补丁)

2026/9/17
  * 修复安卓编译失败的问题(感谢智明提供补丁)
  * 把nanovg_plus的15份ubo的机制改为同一份ubo（为了解决intel部分显卡驱动执行多份ubo会崩溃的问题）(感谢智明提供补丁)
  * CMake同步harfbuzz的修改(感谢均权提供补丁)
...
2026/9/10
  ...
  * 完善 ui_builder 删除不需要的成员(感谢朝泽提供补丁)
  * 时间类型改成64位(感谢兆坤提供补丁)
  * 修复全屏后鼠标光标位置发生变化的问题(感谢兆坤提供补丁)
  * 同步 font_t 中 name 的长度(感谢泽武提供补丁)
  * 0x7fffffff 和 0xffffffff 用标准宏替换(感谢兆坤提供补丁)

2026/9/9
  * 统一bsearch实现(感谢兆坤提供补丁)
  * 修复combo_box同步文本到index时没有从value控件获取的问题(感谢福明提供补丁)
  * 统一bsearch实现并完善相关代码(感谢兆坤提供补丁)
...
2026/9/3
  ...
  * 打包脚本添加 IS_SAVE_SPECIAL_CHAR 属性，用于 font_gen 打包时的特殊字符保留(感谢泽武提供补丁)
  * 完善 button 和 calibration_win 控件的 ellipses 属性的实现(感谢泽武提供补丁)
  * 修复部分 xml 中存在属性重复定义的问题(感谢泽武提供补丁)
...
2026/8/21
  ...
  * 修复 vgcanvas 裁剪区可能异常的问题 (感谢雪飞提供补丁)
...
2026/8/17
  * 完善 edit (感谢雪飞提供补丁)
    - 优化删除文本时可能看不见删除效果的问题
    - 使 edit 尽可能多的显示文本，避免右侧空白
  * 修复 AWStudio 中鼠标指针在不同组件中切换焦点后，点击编辑框会出现指针样式异常的问题(感谢雪飞提供补丁)
...
2026/8/13
  ...
  * 修复离线画布释放之后导致离线画布的位图重复刷新显存数据的问题(感谢智明提供补丁)

2026/8/12
  * 修复demo由于运行目录下缺少res\assets而无法加载资源的问题(感谢均权提供补丁)
  * 修复mingw下clang编译器编译问题(感谢均权提供补丁)
  * fix edit_ex(感谢兆坤提供补丁)

2026/8/11
  * 控件弹出popup时不丢失焦点(感谢雨欣提供补丁)
...
2026/8/3
  ...
  * 修复 always_on_top 窗口被重复绘制的问题(感谢雪飞提供补丁)

2026/7/31
  * 修复高亮残留 dialog 下面的 always_on_top 窗口的问题(感谢雪飞提供补丁)
  * 修复高亮残留 dialog 上面的窗口的问题(感谢雪飞提供补丁)
  * 增加 object_fifo (感谢福明提供补丁) 
  * 设置focus_next_when_enter为false时禁用上下方向键切换焦点(感谢福明提供补丁)

2026/7/29
  * 完善 shift 输入，以修复以下两个问题(不限于@，其他需要shift键入的字符都行)(感谢雪飞提供补丁):
    - 有软键盘的情况下，使用物理键盘在文本框A快速输入 @ (尽可能快的完成按下shift、按下2、松开shift、松开2) ，然后关闭软键盘后再点击文本框B并长按 shift，此时首次按下 2 会输入无效
    - 在上述场景中，切换到文本框B后，如果始终快速输入 @ 会一直输入无效
...
2026/7/27
  * 修复在打开键盘的情况下从overlay1切换到(通过点击非edit区域)overlay2时，overlay2会出现跳变的问题(感谢雪飞提供补丁)
  * 修复在键盘完全关闭后，再打开edit, 窗口可能被额外抬升的问题(感谢雪飞提供补丁)

2026/7/24
  * 完善打开 popup(感谢雪飞提供补丁)
    - 修复只读下拉列表与 overlay 叠加时，点击文本区域打开下拉列表并关闭后, 松开鼠标还能拖拽选择文本的问题
    - 修复下拉列表与 overlay(带edit) 叠加时，点击下拉列表并关闭后，此时 edit 无法输入的问题
  * 修复下拉列表与 overlay(带edit) 叠加时，通过edit打开键盘，再点击下拉列表并关闭 后会打开一个无效键盘的问题(感谢雪飞提供补丁)
  * 修复鼠标进入页面时 scroll_bar 不显示，以及显示不全的问题(感谢雪飞提供补丁)

2026/7/22
  * 修复XMLparser报错处理时列号不正确的问题(感谢福明提供补丁)
  * 修复编译问题(感谢文静提供补丁)

2026/7/20
  * 完善 XML parser的报错处理(感谢朝泽提供补丁)
  * 修复能操作模态对话框后面的窗口的问题(感谢雪飞提供补丁)
  * 完善窗口切换(感谢雪飞提供补丁)
      - 修复 demoui 中 edit 放到 system bar 里会无法输入的问题
      - 修复 demoui 中打开键盘时切换到其他页会出现键盘闪烁以及部分情况下切换后窗口位置异常的问题
      - 修复 demoui 的 Window 页中打开 overlay, 再切到其他页再切回来，此时打开 dialog 再关闭后页面 yoffset 异常的问题
...
2026/7/14
  * 修复self_layout="flex(x=c,y=m,w=600,h=flex)"，没有居中的问题(感谢兆坤提供补丁)

2026/7/10
  * 修复 edit/mledit 在没有焦点的情况下能打开键盘的问题(感谢雪飞提供补丁)
  * 修复 edit 一部分超出窗口下边界时打开键盘会产生图像残留的问题(感谢雪飞提供补丁)

2026/7/9
  * 修复键盘打开时切换不同键盘类型的编辑器导致键盘无法关闭且焦点锁死在切换后的编辑器上的问题(感谢雪飞提供补丁)
  ...
  * edit_ex support multiline(感谢兆坤提供补丁)
...
2026/7/3
  * 修复了多行编辑器光标位置异常导致无法正常输入的问题(感谢雪飞提供补丁)
...
2026/6/16
  ...
  * 完善了使控件滚动到可见区域的默认算法 (感谢雪飞提供补丁)
    - 选择最短的路径移到可视区域
    - 优化控件一侧刚好位于可见区域边缘的情况，避免"拉风箱"
...
2026/6/10
  * 完善 object_evt_router(感谢兆坤提供补丁)
  * 修复组件页面theme属性为空，但存在同名样式，预览时没使用同名样式的问题(感谢培煌提供补丁)
  * 修复在agge下对话框截屏时由于高亮策略没有在正确的canvas上绘制导致截屏结 果没有显示背景窗口的问题(感谢福明提供补丁)
...
2026/6/8
  ...
  * 完善 object_evt_router(感谢兆坤提供补丁)
  * 修复了在某些情况下 im->busy 没有正常复位导致无法打开软键盘的问题(感谢雪飞提供补丁)
  * 修复了软键盘的两个异常行为：重复点击当前编辑框会导致已打开的软键盘关闭、点击只读编辑框会打开软键盘(感谢雪飞提供补丁)

2026/6/5
  * 完善 assert_catcher.hpp (感谢兆坤提供补丁)
  * 补充调试打印(感谢兆坤提供补丁)
  * 补充调试打印(感谢兆坤提供补丁)
  * 完善了 edit_date 对日期的校验(感谢雪飞提供补丁)
...
2026/6/03
  * 修复拼写错误(感谢兆坤提供补丁)
  * 完善支持.otf字体(感谢培煌提供补丁)
  * 去除edit_ex中不必要的断言(感谢俊杰提供补丁)

2026/6/02
  * 修复快速地按按键，让popup频繁打开关闭，可能会出现edit失焦的问题(感谢兆坤提供补丁)
  * 完善从组件页面中获取theme属性值的逻辑(感谢培煌提供补丁)

2026/6/01
  * 修复 button.xml 拼写错误(感谢俊杰提供补丁)
  * 修复当组件页面指定了theme属性时预览没有使用指定的样式的问题(感谢培煌提供补丁)

2026/5/29
  * 完善edit ex(感谢兆坤提供补丁)
    - 修复item只有一项时，看不到下面的按钮
    - 不再将 auto_hide_scroll_bar = true 设置为默认值

  * edit ex 在移动端的效果(感谢兆坤提供补丁)
  * 修复路径判断有误导致预览时可能组件无显示的问题(感谢培煌提供补丁)

2026/5/28
  * 修复 keil 编译出错的问题(感谢兆坤提供补丁)
  * 增加 edit_ex_set_suggest_words_popup_on_updated 函数(感谢兆坤提供补丁)
  * 修复keil编译出错的问题(感谢智明提供补丁)
...
2026/5/20
  ...
  * 移除 AGG 后端残留（NANOVG_BACKEND=AGG 选项、demo 及文档提及已清理，软件渲染仅保留 AGGE）
  * 修复了 socket_helper 在 c++ 中编译时可能出现链接错误的问题(感谢雪飞提供补丁)

2026/5/18
  * 调整tk_object_t内存布局(感谢兆坤提供补丁)
  * 修复widget last_state_for_style 可能是野指针的问题(感谢兆坤提供补丁)
  * 修复当消息剩1条时dialog_toast_quit可能访问野指针的问题(感谢朝泽提供补丁)
  * 修复candidate控件没有正确获取字体名称的问题(感谢泽武提供补丁)
...
2026/5/12
  ...
  * 修复了使用串口service时会有内存泄漏的问题(感谢雪飞提供补丁)
  * 完善 mem_allocator_fixed_block (感谢兆坤提供补丁)

2026/5/11
  * 修复在非默认主题预览ui时，如果component视图在默认主题中，会无显示的问题(感谢培煌提供补丁)

2026/5/9
  * 完善 value 浮点数转整数(感谢兆坤提供补丁) 
  * 增加 file_write_ex/file_write_sync_ex (感谢智明提供补丁)
  * 修复派发global_emitter上注册的事件时事件对象中没有ctrl等修饰键的信息的问题(感谢福明提供补丁)

2026/5/7
  * 修复 value 中 double 转换成 int64 与 uint64 可能出现的截断问题(感谢泽武提供补丁)
  * 修复了Windows下串口分段读取数据总是超时的问题(感谢雪飞提供补丁)
  * 完善 tk_object_to_json 缩进和换行格式(感谢兆坤提供补丁)

2026/05/06
  * 修复 支持 SDL3 引入 按键事件重复定义导致键盘无法输入的问题(感谢兆坤发现问题)
  * 实现按shift水平滚动(感谢兆坤提供补丁)

2026/05/03
  * bidi 算法默认使用 SheenBidi
...
2026/04/30
  * 修复 edit_ex 销毁后触发了 popup 的事件导致崩溃的问题(感谢兆坤提供补丁)
  * 修复 edit_ex 打开 popup 后关闭程序的崩溃问题(感谢兆坤提供补丁)
...
2026/04/15
  ...
  * 增加鼠标额外按键事件(感谢兆坤提供补丁)
  * 修复 Ubuntu 上 SRC_DIR 带 ; 时执行生成 idl.json 的命令行会失败的问题(感谢朝泽提供补丁)
...
2026/04/13
  * 完善输出文本解码，避免可能因文本编码有误而导致打包中断(感谢培煌提供补丁)

2026/04/9
  * 完善 value_compare(感谢泽武提供补丁)

2026/04/8
  * 修复由于未设置 -std=c++11 导致 awtk 单元测试在 Ubuntu 上编译失败的问题(感谢朝泽提供补丁)
  * 完善了 text_edit 的参数检查(感谢雪飞提供补丁)

2026/04/7
  * 修复 typed_array_extend_delta(感谢兆坤发现问题)
  * 优化 typed_array 效率 (感谢兆坤提供补丁)
  * 完善 typed_array (感谢兆坤提供补丁)
  * 修复因根标签解析有误导致可能子控件无显示的问题(感谢培煌提供补丁)
  * 输入法预编辑区添加了一个默认字号(感谢雪飞提供补丁)

2026/04/5
  * 完善文档(感谢兆坤提供补丁)

2026/04/3
  * demo_5key 在设置键盘类型断言失败时添加了错误提示(感谢雪飞提供补丁)
  * 改善了输入法预编辑区的文本大小(感谢雪飞提供补丁)
  * 修复使用默认参数编译agge出错的问题(感谢智明提供补丁)
  * 修复 agge 绘制渐变的时候触发断言的问题(感谢智明提供补丁)

2026/04/2
  * 增加函数 tk_count_lines / tk_wstr_count_lines 
  * 修正了hscroll_label控件修改宽度为小于字符长度，内部的字体不会滚动的问(感谢陈谭/雪飞提供补丁)
  * 修正了在单行和多行编辑器使用系统输入法时，输入法候选字窗口位置不跟随焦点光标的问题(感谢陈谭/雪飞提供补丁)
...
2026/03/31
  * 修复拼写错误(感谢兆坤提供补丁)
  * 修复圆角矩形计算脏矩形时xy没有计算线宽的问题(感谢雨欣提供补丁)

2026/03/30
  * 修复popup的active_window_by_parent_widget比popup先销毁时，会发生崩溃的问题(感谢兆坤提供补丁)

2026/03/27
  * 完善zip_file_t的注释(感谢福明提供补丁)
  * object_locker支持clear_props(感谢雨欣提供补丁)
  * 导出 SDL_SetTextInputRect(感谢俊圣提供补丁)
...
2026/03/22 
  * 完善 object_evt_router(感谢兆坤提供补丁)
    - 1.增加发布者订阅者生命周期选项
    - 2.在发布过程中允许退订
    - 3.补充log，并且将log从打印改为分发事件
  * 完善lf_bp_buffer包含的头文件(感谢雨欣提供补丁)
...
2026/03/19
  ...
  * 修复当父控件设置了弹性布局，子控件位置和大小发生改变后，父控件没有重新布局的问题(感谢兆坤提供补丁)

2026/03/18
  * preview_ui 添加 theme 的默认值设置(感谢泽武提供补丁)
  * WIDGET_PROP_ANIMATE_PREFIX 的处理(感谢泽武提供补丁)
  * 完善 ui_loader_xml ：(感谢泽武提供补丁)
   -（1）修改 xml 中控件属性写入的时机为读取完所有属性后一次性写入
   -（2）修改 <?include?> 标签中覆盖属性的逻辑为替换原控件的对应属性
   -（3）修改当同时存在 self_layout 和 xywh 为非像素模式时的覆盖逻辑，以后方的属性为准
  * 增加 object_evt_proxy(感谢兆坤提供补丁)

2026/03/16
  * 修复object_default、object_hash切换大小写是否敏感的设置时，里面的元素没有重排导致二分查找失效的问题(感谢兆坤提供补丁)
  * 新增use_virtual_h和use_widget_h，支持用户控制布局使用的高度(感谢雨欣提供补丁)

2026/03/14
  * 优化 object_hash 效率 (感谢兆坤提供补丁)
  * 修复窗体名称设置错误导致没有正确加载同名样式资源的问题(感谢培煌提供补丁)
...
2026/02/02
  * 修复编译警告(感谢兆坤提供补丁)
  * 修复edit_ex mvvm text 属性双向绑定的问题(感谢兆坤提供补丁)
  * 修复target移动popup后，edit_ex无法使用快捷键的问题(感谢兆坤提供补丁)

2026/01/28
  * 增加 object_override_set_base_obj 函数(感谢兆坤提供补丁)

2026/01/27
  * 增加 tk_await 函数(感谢兆坤提供补丁)

2026/01/22
  * 修复 awtk 的 nanovg_plus 判断绘图矩形裁剪区有问题，导致某些情况下矩形画不出来(感谢智明提供补丁)
...
2026/01/16
  * 增加 object_load_conf (感谢兆坤提供补丁)
  * object_load_conf 可禁用路径访问 (感谢兆坤提供补丁)
  * 完善测试用例 (感谢兆坤提供补丁)

2026/01/15
  * 完善 yaml 引号字符串、扩展键名、布尔值支持、Null 值支持、流式集合支持和多行文本等功能(感谢兆坤发现问题)
  * 完善conf_yaml_test可读性 (感谢兆坤提供补丁)
  * 完善 yaml 节点类型(感谢兆坤发现问题)

2026/01/14
  * 增加conf_yaml_load_ex  (感谢兆坤提供补丁)

2026/01/12
  * 删除重复代码(感谢智明提供补丁)

2026/01/08
  * 修改label的length属性在换行模式下生效(感谢朝泽提供补丁)

2026/01/06
  * 修复mingw环境下中文路径打包失败问题和当传入空字符目录路径时陷入死循环的问题(感谢培煌提供补丁)
...
2025/12/31
  * 修复注释错误(感谢陈谭提供补丁)
  * 完善 object_compositor (感谢兆坤提供补丁)

2025/12/29
  * 修改children_layout_default中统一限制子控件的宽高最小为0，避免如网格布局时子控件的宽或高出现极大值的问题(感谢朝泽提供补丁)
  * 修复native_windows句柄引用计数器不正确的问题(感谢智明提供补丁)
  * 修复app在编译的时候无法加载app目录下的awtk_config_define.py文件(感谢智明提供补丁)

2025/12/23
  * 修复graphic_buffer设置逻辑高的参数合法性验证错误(感谢泽武提供补丁)
  * 添加行号、偏移位置的转换接口，以及修改控件大小变化时保持当前显示的第一行不变(感谢朝泽提供补丁)

2025/12/20
  * 修正了Win32中部分USB转串口设备调用WaitCommEvent返回ERROR_INVALID_PARAMETER的问题，每次WaitCommEvent前要重置CommMask(感谢陈谭提供补丁)

2025/12/18
  * 完善 date_time_to_time_impl (感谢兆坤发现问题)

2025/12/18
  * 将pointer_event_t放在一起(感谢兆坤提供补丁)
  * edit_ex：修复建议词弹框打开后，鼠标无法操作编辑框的问题(感谢兆坤提供补丁)
...
2025/12/9
  * 修正conf_json_load_ex参数没生效的问题(感谢福明提供补丁)
  * 去除 xml 文件中单个标签内的键值对数量上限(感谢泽武提供补丁)
  * preview_ui 添加识别当前待预览的文件是否为窗口的功能(感谢泽武提供补丁)
  * xml 中 <?include?> 标签添加新功能：可通过控件名称与属性名称来修改被包含的文件中控件的属性和样式，语法规则为：prop_name-prop="xxx" (感谢泽武提供补丁)
  * 去除 xml 中 <?include?> 标签对文件格式的限制 (感谢泽武提供补丁)

2025/12/4
  * 修复 atomic: remove bit (感谢兆坤提供补丁)
  * 完善 atomic(感谢兆坤提供补丁)
  * 完善 demoui popup (感谢兆坤提供补丁)
  * 修复子进程读取数据提前中断的问题(感谢智明提供补丁)
...
2025/11/20
  * 完善 text_edit 的整词删除(感谢泽武提供补丁) 
  * 修复 text_edit 修复整词删除功能 (感谢兆坤提供补丁)
...
2025/11/11
  * 完善edit_ex(感谢兆坤提供补丁)
  * 修复 log (感谢兆坤提供补丁)
  * 修复xml中的语法错误 (感谢福明提供补丁）

2025/11/6
  * 完善函数原型(感谢兆坤提供补丁)
  * 完善edit_ex控件(感谢雨欣提供补丁)

2025/11/5
  * 去掉 WITH_NANOVG_AGG (感谢兆坤发现问题）

2025/11/4
  * 完善widget_set_self_layout_params接口(感谢雨欣提供补丁)
  * 向量化优化(感谢兆坤提供补丁)
  * 优化效率(感谢兆坤提供补丁)
...
2025/10/27
  * 修正了mingw使用-std=c++11/17编译时，缺少WIN32宏定义问题，并删除多余的_WIN32系统宏定义（编译器会自动定义）(感谢陈谭提供补丁)
...
2025/10/20
  * 完善 scroll_view 
  * gif_image 添加边播放边加载模式(感谢泽武提供补丁)

2025/10/17
  * 完善widget的事件注释(感谢福明提供补丁)
...
2025/10/14
  * 修复3_5keys模式下打开键盘后会自动按下键盘上的焦点按键（linux_fb：issue\#119）(感谢兆坤提供补丁)
  * 完善3_5keys(感谢兆坤提供补丁)
  * 修复accept_button按下时，final_key_target找不到后的崩溃问题(感谢兆坤提供补丁)

2025/10/11
  * 增加函数 tk_ulltoa(感谢志鹏提供补丁)
  * 完善demouiold(感谢泽武提供补丁)
  * 完善测试(感谢兆坤提供补丁)
...
2025/09/28
  ...
  * 完善 scroll_bar（感谢俊杰提供补丁)

2025/09/27
  * 修复还没有完成窗口动画的窗口马上被关闭会导致窗口动画结束崩溃的问题(感谢智明提供补丁)
  * 修复在屏保的回调函数中再次修改屏保时间出现无限触发屏保回调的问题(感谢智明提供补丁)
  * 修复定时器的回调函数中添加一个相同的定时器导致出现死循环的问 题(感谢智明提供补丁)
  * 回退屏保补丁(感谢智明提供补丁)

2025/09/26
  * rich text 支持 word_wrap 属性。

2025/09/23
  * 完善文档(感谢泽武提供补丁)
  * 完善line parser(感谢泽武提供补丁)

2025/09/22
  * 完善widget支持的事件注释(感谢福明提供补丁)

2025/09/17
  * 增加edit_get_int64（感谢高源提供补丁)
...
2025/09/11
  ...
  * 增加获取子进程退出code的函数(感谢智明提供补丁)
  * 完善scroll view(感谢俊杰提供补丁)
  * 增加子进程显示窗口的参数(感谢智明提供补丁)
  * idle_queue失败时触发断言并打印警告(感谢雨欣提供补丁)

2025/09/10
  * 完善locale_info_xml(感谢朝泽提供补丁)
  * 完善字符串是否为数字的判断(感谢兆坤提供补丁)
  * 完善 slist_get/set(感谢兆坤提供补丁)
  * 增加写入文件后立马同步的函数(感谢智明提供补丁)

2025/09/09
  * 增加判断字符串是否为数字的函数(感谢兆坤提供补丁)

2025/09/08
  * 完善生成位图字体资源时，如果当前主题中找不到原始字体文件，则从默认主题中找(感谢培煌提供补丁)
...
2025/09/04
  * 完善 str (感谢兆坤提供补丁)
  * 增加 str_escape (感谢兆坤提供补丁
  * 去掉部分不必要的参数判断，以及减少不必要的打印(感谢朝泽提供补丁)
  * 修改text edit在style准备好后再layout(感谢朝泽提供补丁)
  * 补充list_view复制控件时需要复制的属性(感谢培煌提供补丁)
  * 完善 mledit (感谢朝泽提供补丁)

2025/09/03
  * 完善生成位图字体时从配置文件中获取位图字体bpp属性(感谢培煌提供补丁)
  * 增加revertible_str类和对应的测试用例(感谢智明提供补丁)

2025/09/02
  * 完善注释(感谢兆坤提供补丁)
  * 完善value_compare(感谢兆坤提供补丁)

2025/09/01
  * 支持时间tick为32位的时候依旧支持运行正常的情况(感谢智明提供补丁)

2025/08/30
  * 修复fbo使用多次重采样后导致获取位图数据失败的问题(感谢兆坤提供补丁) 

2025/08/29
  * 完善 foreach (感谢兆坤提供补丁)
  * improve widget_foreach (感谢兆坤提供补丁)
...
2025/08/22
  * 完善tree_node_feature_segment(感谢兆坤提供补丁)
  * 完善注释(感谢兆坤提供补丁)
  * 修复OpenGL的convex_fill不处理点顺时针不绘图的问题(感谢智明提供补丁)
...
2025/08/7
  * 增加 线段树结点特征(感谢兆坤提供补丁)
  * 补充scroll_bar复制控件时需要复制的属性(感谢培煌提供补丁)
...
2025/08/4
  ...
  * 完善测试(感谢兆坤提供补丁)
  * 修复edit控件不支持用户自定义key_up_RETURN事件(感谢志鹏提供补丁)
  * 修复widget控件中几个事件分发时的问题(感谢志鹏提供补丁)
...
2025/07/21
  ...
  * 修复无法启动多重采样抗锯齿的情况下自动切换为软件抗锯齿(感谢智明提供补丁)。
...
2025/07/17
  * 完善edit_ex(感谢兆坤提供补丁)
  * 修复 edit state 设置有误的问题(感谢兆坤提供补丁)
  * 修复圆角矩形在粗边框的情况下和填充大小对不上的问题(感谢智明提供补丁)
  * 修改scons脚本支持在windows和osx平台下做linux-fb的交叉编译(感谢陈谭提供补丁)
...
2025/07/15
  ...
  * 去掉布局时的无用的警告(感谢兆坤提供补丁)。

2025/07/14
  * 移除EVT_WIDGET_RESTACK_CHILD的事件注释(感谢福明提供补丁)
  * 修复pages控件销毁时没有移除idle的问题(感谢福明提供补丁)

2025/07/09
  * fscript 支持 obj.exec/obj.exec_ex

2025/07/07
  * 修复一条贝塞尔曲线填充的时候出现多次叠加绘图的问题（nanovg_plus版本）(感谢智明提供补丁)。

2025/07/07
  * 增加函数 value_compare (感谢兆坤提供补丁)。 
  * 修复一条贝塞尔曲线填充的时候出现多次叠加绘图的问题(感谢智明提供补丁)。 

2025/07/04
  * 修复keyboard被销毁后，依然触发定时器强制关闭keyboard导致的崩溃问题(感谢雨欣提供补丁)
...
2025/06/24
  * 完善 typed_array (感谢兆坤提供补丁)。
  * 修复dx11在某些电脑下隐藏状态出现显存泄露的问题(感谢智明提供补丁)。
  * 修复mono缺少lcd_sdl2_texture_reset函数导致编译出错的问(感谢智明提供补丁)。

2025/06/20
  * 修复使用sdl不同的Render（D3D11或者D3D9）有可能黑屏的问题(感谢智明提供补丁)。

2025/06/19
  * 修改mingw编译时动态库按def文件导出符号(感谢朝泽提供补丁)

2025/06/18
  * 修复agge模式下画线，如果第一条路径可忽略时，后面的路径都没有绘制的问题(感谢兆坤提供补丁)。 
  * 完善label控件中的length属性文档(感谢俊杰提供补丁)
  * 完善slider控件中的drag_threshold属性文档(感谢俊杰提供补丁)
  * children_layouter_default指定子控件的宽高后，不再根据行列数重新计算子控件的宽高(感谢兆坤提供补丁)。
...
2025/06/16
  ...
  * 完善 注释(感谢兆坤提供补丁)。
  * 给vg增加填充模式和svg支持非零规则填充模式(感谢智明提供补丁)
  * 完善 window_animator_vtranslate(感谢俊杰提供补丁)
  * 完善 mutable image(感谢俊杰提供补丁)

2025/06/14
  * 完善 demoui (感谢兆坤提供补丁)。
  * 完善 scrollview (感谢兆坤提供补丁)。
  * 完善 宏定义(感谢兆坤提供补丁)。

2025/06/13
  * 增加 tk_isalnum
  * 增加 self_layouter_flex (感谢兆坤提供补丁)。
  * 当虚拟宽高变小时，偏移位置不能大于调整后的虚拟宽高(感谢兆坤提供补丁)。
  * 完善 layouter(感谢兆坤提供补丁)。
  * 完善 self_layouter_menu (感谢兆坤提供补丁)。

2025/06/11
  * 完善 layouter(感谢兆坤提供补丁)。

2025/06/10
  * 修复xml中控件设置auto风格失败的问题(感谢智明提供补丁)
...
2025/06/05
  * 修复当水平滚动条和滚动窗口宽度不一样时，虚拟宽度设置错误的问题（感谢兆坤提供补丁)。
  * 完善 list view/scroll bar （感谢兆坤提供补丁)。
  * 修复candidates获取enable_preview失败的问题（感谢福明提供补丁）
...
2025/05/29
  ...
  * 取消"修复pages控件的auto_focused属性不生效的问题"的修改（感谢俊杰提供补丁)。
  * 修复hscroll_label控件设置yoyo=TRUE，stop_at_begin属性还生效的问题（感谢俊杰提供补丁)。
...
2025/05/28
  ...
  * 修复slide_menu控件下面问题（感谢俊杰提供补丁)。
     ...
     - 修改clip属性，控件不会立即更新。
     - 修改menu_w属性，控件不会立即更新。
  * 增加串口的枚举转化为字符串的接口和测试用例(感谢智明提供补丁)
  * 废弃 default_focused_child 属性（感谢俊杰提供补丁)。
...
2025/05/20
  * 完善打包脚本，兼容python2.7(感谢培煌提供补丁)

2025/05/19
  * 完善 edit_ex (感谢兆坤提供补丁)。
  * 完善fscript对象生命周期管理(感谢兆坤提供补丁)。
  * 修复edit加载属性顺序不同导致自动修复功能显示不正常的问题(感谢智明提供补丁)

2025/05/17
  * 修复combo_box设置了focus_next_when_enter属性不生效（感谢俊杰提供补丁)
...
2025/04/03
  * 完善tree(感谢兆坤提供补丁)
  * 修复编译警告(感谢智明提供补丁)
  * 增加wbuffer_clone函数和对应的测试用例(感谢智明提供补丁) 
  * 给client和service增加注释(感谢智明提供补丁)
  * 增加client/serice测试程序。

2025/04/02
  * 完善tree测试程序(感谢兆坤提供补丁)
  * 优化main_loop_sdl对退格符的处理，在使用按键触发退格符时拦截退格字符的发送(感谢泽武提供补丁)
  * 完善tree_to_string(感谢兆坤提供补丁)
  * 修复conf node内存泄露
  * 完善tk_service修复可能存在的内存泄露。
  * 修复tree内存泄露(感谢兆坤提供补丁)


2025/04/01
  * 增加tree_to_string(感谢兆坤提供补丁)
  * 修正edit_ex在选择建议词后的光标位置(感谢兆坤提供补丁)

2025/03/31
  * 添加remote_ui客户端收到notify时的回调函数(感谢福明提供补丁)
  * 完善tree(感谢兆坤提供补丁)
...
2025/03/26
  * 修改remote_ui添加处理事件的回调来处理自定义事件(感谢福明提供补丁)
...
2025/03/20
  * 完善单元测试(感谢泽武提供补丁)
  * 修复SDL屏幕纹理大于w乘以bpp的情况导致画面异常(感谢智明提供补丁)

2025/03/19
  * 修复在大分辨率下窗口动画有可能出现回弹的情况(感谢智明提供补丁)
  * 修复edit在 ipv4,date,time模式下光标位置异常的问题(感谢泽武提供补丁)
  * ext_edit_paste函数添加识别单个退格符与删除符，解决开启输入法智能标点时，在中文模式下输入数字加符号出现没有删除中文符号的问题(感谢泽武提供补丁)
  * 修复mledit在使用方向键上下移动光标时，在某些情况下移动到末尾后无法向上移动光标的问题；统一方向键上下在最初始行与最末尾行的逻辑为跳到最开头与跳到最末尾(感谢泽武提供补丁)

2025/03/17
  * 完善demo资源(感谢兆坤提供补丁)
  * 增加agge的RGB565的位图支持(感谢智明提供补丁)

2025/03/13
  * 完善注释(感谢兆坤提供补丁)
  * 控件设置文本后光标重置到文本头部(感谢雨欣提供补丁)
  * 完善scroll_bar_t的注释(感谢雨欣提供补丁)
  * 完善单元测试(感谢雨欣提供补丁)

2025/03/11
  * 修复普通 lcd 旋转后 graphic_buffer 的数据和 bitmap 对不上的问题(感谢智明提供补丁)
  * 修复了nanovg镂空的逻辑，dir是布尔值取值0-1，但是子类需要值为2时才能镂空，要做一个转换(感谢颖健提供补丁)
...
2025/02/20
  ...
  * edit_ex 完善多格式功能2(感谢兆坤提供补丁)
  * edit_ex 去除 format 名字中去除首尾的空格和换行符(感谢兆坤提供补丁)

2025/02/19
  * 修复widget_clone产生的警告(感谢兆坤提供补丁)
  * 完善打印log(感谢兆坤提供补丁)
  * edit_ex 可以根据format生成item内容(感谢兆坤提供补丁)
...
2025/01/13
  ...
  * 修复当页面内嵌小程序页面时，tab键切换焦点只能在小程序页面内切换的问题(感谢培煌提供补丁)
